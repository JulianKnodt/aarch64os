...
impl DeviceTree {
  pub unsafe fn from_address<'a>(base: *const Self) -> &'a Self { &*base }

  pub fn nodes(&self) -> NodeIterator {
    unsafe {
      NodeIterator {
        struct_base: (self as *const _ as *const u8).offset(self.dt_struct_offset.native() as isize)
          as *const BE,
  ...
  pub fn root(&self) -> Option<Node> { self.nodes().next() }
}

...
macro_rules! impl_has_endianness {
  ($t: ty) => {
    impl HasEndianness for $t {
      ...
      fn to_be(self) -> Self { <$t>::from_be(self) }
      fn to_le(self) -> Self { <$t>::from_le(self) }
    }
  };
}
...
impl<T, E> Endian<T, E> {
  pub const fn from_raw(native: T) -> Self { Endian(native, core::marker::PhantomData) }
}

impl<T: HasEndianness, E: Endianness> Endian<T, E> {
  pub fn new(native: T) -> Self { Endian(E::from_native(native), core::marker::PhantomData) }

  pub fn native(self) -> T { E::to_native(self.0) }
...
impl Endianness for Big {
  fn from_native<T: HasEndianness>(native: T) -> T { native.to_be() }

  fn to_native<T: HasEndianness>(native: T) -> T { native.from_be() }
}

impl Endianness for Little {
  fn from_native<T: HasEndianness>(native: T) -> T { native.to_le() }

  fn to_native<T: HasEndianness>(native: T) -> T { native.from_le() }
}

impl<T: HasEndianness, D: Endianness> From<T> for Endian<T, D> {
  fn from(f: T) -> Self { Self::new(f) }
}

impl<T: HasEndianness + fmt::Display, D: Endianness> fmt::Display for Endian<T, D> {
  fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result { write!(f, "{}", D::to_native(self.0)) }
}
...
impl<T: HasEndianness + fmt::LowerHex, D: Endianness> fmt::LowerHex for Endian<T, D> {
  fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result { write!(f, "{:#x}", D::to_native(self.0)) }
}

    ...
    [(); OWN_BLOCKS * B::BLOCK_SIZE]:,
{
    ...
    pub fn try_init(&mut self) -> Result<(), InitErr> {
        ...
        for i in 0..MD_SPACE {
            ...
            let len = id.len();
            self.stored[i] = Some(
                AllMetadata::de(id, &buf[curr..curr + len])
                    .map_err(|_| InitErr::FailedToInitMetadata)?,
            );
            curr += len;
        }
        Ok(())
    ...
    pub fn free_map(&self) -> &BitArray<{ B::NUM_BLOCKS }> {
        &self.free_map
    }
    ...
    #[cfg(test)]
    pub(crate) fn block_device_mut(&mut self) -> &mut B {
